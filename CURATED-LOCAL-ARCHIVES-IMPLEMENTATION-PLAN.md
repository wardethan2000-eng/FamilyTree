...
### Phase 1: Refactor Existing Full Export — ✅ COMPLETE
...
- [x] Extract `ArchiveExportManifest` types → `apps/api/src/lib/archive-export/types.ts`
- [x] Extract full-tree manifest builder → `apps/api/src/lib/archive-export/manifest-builder.ts`
- [x] Extract person-scoped manifest builder → `manifest-builder.ts` (`buildPersonManifest`)
- [x] Extract ZIP writer → `apps/api/src/lib/archive-export/zip-writer.ts`
- [x] Extract offline viewer renderer → `apps/api/src/lib/archive-export/html-renderer.ts`
- [x] Extract media export module → `apps/api/src/lib/archive-export/media-export.ts`
- [x] Replace media filenames with stable media-ID filenames (`media/<uuid>.<ext>`)
- [x] Include transcript text in export data
- [x] Include all memory media items, not only primary media
- [x] Include perspectives in memory data
- [x] Include places in manifest
- [x] Include `alsoKnownAs`, relationship types (`spouseStatus`, `startDateText`, `endDateText`)
- [x] Add `<noscript>` fallback in viewer
- [x] Add README.txt in ZIP
- [x] Add search bar in viewer
- [x] Add memory detail view (click through from person page)
- [x] Add video/audio playback support
- [x] Add transcript display
- [x] Add perspective display
- [x] Hash-based navigation ready (viewer JS uses `showPerson`/`showMemory` — hash routing comes with Preact viewer)
- [x] Permissions block uses `exportedByUserId`/`exportedByRole` (not `viewerUserId`)
- [x] `output_kind` enum limited to `full_zip` | `mini_zip` (no `share_link` until Phase 6)
- [x] `archive_exports` extended with `collection_id`, `output_kind`, `media_quality`, `manifest_version`, `manifest_json`, `error_code`, `error_message`

Files created:

- `apps/api/src/lib/archive-export/types.ts`
- `apps/api/src/lib/archive-export/manifest-builder.ts`
- `apps/api/src/lib/archive-export/media-export.ts`
- `apps/api/src/lib/archive-export/zip-writer.ts`
- `apps/api/src/lib/archive-export/html-renderer.ts`
- `apps/api/src/lib/archive-export/index.ts`
...
- [x] Add `archive_collections` schema with `slug`, `scope_kind`, `scope_json`, `default_view_mode`, `visibility`, `include_relationships`, `include_related_memories`, `include_places`
- [x] Add `archive_collection_sections` schema with `section_kind`, `sort_order`, `settings_json`
- [x] Add `archive_collection_items` schema with nullable FK columns (`person_id`, `memory_id`, `place_id`, `relationship_id`) instead of polymorphic `item_id`/`item_kind`
- [x] Add collection draft endpoint `POST /api/trees/:treeId/archive-collections/draft` (person scope)
- [x] Add collection CRUD endpoints (GET list, POST create, GET one, PATCH update, DELETE)
- [x] Add item management endpoints (POST add item, DELETE remove item)
- [x] Add manifest preview endpoint `GET /:collectionId/manifest-preview`
...
- [x] A steward can create a local archive for one person (via API)
- [x] The ZIP opens offline by double-clicking `index.html` (current viewer)
- [ ] A viewer can click a memory and see body, media, transcript, tagged people, and related context (current viewer has this but Preact viewer needs it)
- [ ] A viewer can click a related person and see that person's included context (current viewer has this)
- [x] Hidden/unviewable memories are excluded (manifest builder uses `getTreeMemories` with `viewerUserId`)
- [x] Media filenames do not collide (stable `media/<mediaId>.<ext>` paths)
...
1. **Preact for offline viewer** — The viewer will be built with Preact (3KB gzipped) rather than plain DOM template strings. This enables component composition for 5 viewing modes. The existing 340-line string-template viewer becomes unmaintainable at the planned complexity.

2. **Nullable FK columns instead of polymorphic `item_id`/`item_kind`** — `archive_collection_items` uses `person_id`, `memory_id`, `place_id`, `relationship_id` as nullable foreign keys instead of a generic `item_kind`/`item_id` pair. This gives referential integrity, cascade deletes, and direct joins.

3. **`share_link` deferred from `output_kind`** — The `export_output_kind` enum contains only `full_zip` and `mini_zip`. Share links will be added in Phase 6 when the hosted sharing infrastructure is built.

4. **`include_context` replaced with explicit booleans** — `archive_collections` has `include_relationships`, `include_related_memories`, and `include_places` instead of a single `include_context` boolean. Collection-level defaults, per-item overrides (nullable) on `archive_collection_items`.

5. **`viewerUserId` renamed to `exportedByUserId`** — The manifest permissions block records who generated the export and their role, not who will view it. Local ZIPs cannot track viewer identity.

6. **Living subject opt-out: Option A (exclude entirely)** — When a person opts out of exports (Phase 3+ feature), their memories are excluded entirely and their name/photo are removed. Not "include but anonymize."

7. **Media quality: default to `web`** — Exports default to `web` quality (max 2000px JPEG 85). `original` and `small` (800px) are options. Sharp integration deferred — V1 passes through originals.

8. **Synchronous exports for Phase 1-2** — ZIP generation streams directly to the HTTP response. The existing `archive_exports` table supports async but won't be used until Phase 3+ when collections may be large enough to warrant background jobs.

9. **Slug on collections** — `archive_collections` has a `slug` field (auto-generated from name, unique per tree) for use in ZIP filenames and future deep links.

10. **`errorCode` + `errorMessage` on exports** — Structured error codes (`missing_media`, `permission_denied`, `quota_exceeded`) alongside human-readable messages.

### Next Steps (Preact Offline Viewer)

The next implementation session should focus on:

1. **Set up Preact viewer build pipeline** — Vite config at `apps/web/vite.archive-viewer.config.ts` that compiles `apps/web/src/archive-viewer/` to static `viewer.js` + `viewer.css`.

2. **Create viewer components** — `HomeScreen`, `PersonSidebar`, `PersonPage`, `MemoryDetail`, `MemoryCard`, `PersonChip`, `SearchBar`, `DriftMode`. Hook: `useManifest` reads `window.__TESSERA_MANIFEST__`, `useRouter` does hash-based routing.

3. **Update `html-renderer.ts` and `zip-writer.ts`** — Generate HTML shell that embeds the built viewer JS/CSS (or inlines for `file://` compatibility) and injects the manifest JSON.

4. **Extract corkboard** — Move `apps/web/src/components/corkboard/` to `packages/corkboard/`, remove Next.js coupling, accept `memories` prop for offline use alongside the existing `apiBase` + `fetch` path for online use.

5. **Unit tests** — `manifest-builder.test.ts` for permission filtering, person draft scoping, media path stability, hidden memory exclusion.
