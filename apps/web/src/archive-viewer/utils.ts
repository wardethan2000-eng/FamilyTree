import type { ArchiveExportManifest, ExportPerson, ExportMemory, ExportPlace, ExportMedia, ExportPerspective, ExportPersonCuration } from "./types.js";
...
export function getMediaType(manifest: ArchiveExportManifest, mediaId: string | null | undefined): "image" | "video" | "audio" | "unknown" {
...
export function getPlace(manifest: ArchiveExportManifest, placeId: string): ExportPlace | undefined {
  if (!manifest.places) return undefined;
  return manifest.places.find((p) => p.id === placeId);
}
...
export function getRelatedMemories(manifest: ArchiveExportManifest, memoryId: string): ExportMemory[] {
  const memory = getMemory(manifest, memoryId);
  if (!memory || !memory.relatedMemoryIds.length) return [];
  return memory.relatedMemoryIds
    .map((id) => getMemory(manifest, id))
    .filter((m): m is ExportMemory => m !== undefined);
}

export function getMemoriesForPlace(manifest: ArchiveExportManifest, placeId: string): ExportMemory[] {
  return manifest.memories.filter((m) => m.placeId === placeId);
}

export function getPeopleForPlace(manifest: ArchiveExportManifest, placeId: string): ExportPerson[] {
  const personIds = new Set<string>();
  for (const m of manifest.memories) {
    if (m.placeId === placeId) {
      personIds.add(m.primaryPersonId);
      for (const tid of m.taggedPersonIds) personIds.add(tid);
    }
  }
  return manifest.people.filter((p) => personIds.has(p.id));
}
...
export function searchManifest(manifest: ArchiveExportManifest, query: string) {
  ...
  if (!q) return { people: [], memories: [], places: [] };
  ...
  const places = (manifest.places ?? []).filter((p) =>
    p.label.toLowerCase().includes(q)
  );

  return { people, memories, places };
}